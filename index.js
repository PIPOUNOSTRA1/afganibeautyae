...
(function() {
  const getSetting = (key) => {
    return (window.STORE_SETTINGS && window.STORE_SETTINGS[key]) || localStorage.getItem(key) || '';
  };
  ...
  window.fetch = async function(input, init) {
    if (typeof input === 'string' && input.startsWith('/api/')) {
      ...
      // Exclude settings API from Google Sheets redirection (always use Render backend as source of truth)
...
// Function to load settings dynamically from the Render server and update config
async function loadSettingsFromServer() {
  try {
    const apiBase = (window.STORE_SETTINGS && window.STORE_SETTINGS.api_base_url) || (window.location.protocol === 'file:' ? 'http://localhost:8080' : '');
    const url = apiBase ? apiBase.replace(/\/$/, '') + '/api/settings' : '/api/settings';
    
    const res = await fetch(url);
    const settings = await res.json();
    if (settings && typeof settings === 'object') {
      window.STORE_SETTINGS = window.STORE_SETTINGS || {};
      Object.assign(window.STORE_SETTINGS, settings);
      ...
      console.log("Dynamically loaded settings from server successfully.");
    }
  } catch (err) {
    console.warn("Failed to load dynamic settings from server:", err);
  }
}
